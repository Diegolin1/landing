  ...
  return (
    <>
      <div className="bg-brand-900 text-stone-300 font-sans antialiased scroll-smooth transition-colors duration-500 overflow-x-hidden">
        ...
        {/* Hero Section — Aurora Pattern (SaaS 2026) */}
        <header className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-brand-navy-base pt-20 pb-20">
          {/* Parallax Aurora Background */}
          <motion.div 
            className="absolute inset-0 z-0 opacity-40 blur-[80px]"
            animate={{
              background: [
                "radial-gradient(circle at 0% 0%, #0f1a2b 0%, transparent 50%)",
                "radial-gradient(circle at 100% 100%, #9dcbd2 0%, transparent 50%)",
                "radial-gradient(circle at 0% 100%, #1b3d73 0%, transparent 50%)",
                "radial-gradient(circle at 100% 0%, #0f1a2b 0%, transparent 50%)",
                "radial-gradient(circle at 0% 0%, #0f1a2b 0%, transparent 50%)"
              ]
            }}
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
            <motion.div 
              ...
              className="lg:col-span-12 flex flex-col items-center text-center max-w-4xl mx-auto"
            >
              ...
              <form action={formAction} className="flex flex-col sm:flex-row gap-3 w-full max-w-lg mb-16 relative">
                <div className="flex flex-col flex-1 relative">
                  <input 
                    ...
                    className={cn(
                      "bg-white/5 border border-white/20 text-white placeholder-stone-400 px-6 py-4 rounded-full focus:outline-none focus:border-brand-aqua-accent focus:ring-1 focus:ring-brand-aqua-accent transition-all",
                      state?.errors?.email ? "border-red-500/50 focus:border-red-500/50 focus:ring-red-500/50" : ""
                    )}
                  />
                  {state?.errors?.email && (
                    <span className="absolute -bottom-6 left-4 text-[11px] font-bold text-red-400">{state.errors.email[0]}</span>
                  )}
                </div>
                <button
                  type="submit"
                  disabled={isPending}
                  className="bg-brand-orange-cta text-brand-text-primary px-8 py-4 rounded-full font-bold text-[15px] hover:brightness-110 shadow-[0_4px_30px_rgba(255,140,0,0.3)] transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
                >
                  {isPending ? "Solicitando..." : "Acceder Demo"}
                  {!isPending && <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />}
                </button>
              </form>
              
              {state?.success && (<p className="text-brand-aqua-accent text-sm mt-[-2rem] mb-10 font-bold">{state.message}</p>)}
